...
const AssessmentList: React.FC<AssessmentListProps> = ({ assessments, currentUser, setCurrentView }) => (
  <div className="p-6">
    ...
    <div className="flex gap-4 mb-6">
      <button className="bg-indigo-600 text-white px-4 py-2 rounded-lg">All</button>
        ...
        <div key={assessment.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between mb-4">
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${
              assessment.status === 'published' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
            }`}>
              {assessment.status}
            </span>
            <div className="flex items-center gap-2">
              <button className="text-gray-400 hover:text-gray-600" aria-label="View assessment">
                <Eye className="w-4 h-4" />
              </button>
              {currentUser?.role !== 'learner' && (
                <>
                  <button className="text-gray-400 hover:text-gray-600" aria-label="Edit assessment">
                    <Edit className="w-4 h-4" />
                  </button>
                  <button className="text-gray-400 hover:text-red-600" aria-label="Delete assessment">
          ...
          <p className="text-sm text-gray-600 mb-4">Created by {assessment.creator}</p>
          <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
            <span>{assessment.participants} participants</span>
            <span>Avg: {assessment.avgScore}%</span>
          </div>
          <button className="w-full bg-indigo-600 text-white py-2 px-4 rounded-lg hover:bg-indigo-700 transition-colors">
            {currentUser?.role === 'learner' ? 'Take Assessment' : 'View Details'}
          </button>
        </div>
      ))}
    </div>
...
export default function Assessments() {
  ...
  return (
    <>
      ...
      {currentView === 'create-assessment' && <CreateAssessment setCurrentView={setCurrentView} />}
    </>
  );
}
    ...
    return (
      <div className="p-6">
        <div className="mb-6">
          ...
          <p className="text-gray-600">Build a new assessment for your students or candidates.</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div>
              <label htmlFor="assessment-category" className="block text-sm font-medium text-gray-700 mb-2">Category</label>
              <select
                id="assessment-category"
                value={assessmentData.category}
                onChange={(e) => setAssessmentData(prev => ({ ...prev, category: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
                ...
                {assessmentData.questions.map((q, index) => (
                  <div key={index} className="bg-gray-50 p-4 rounded-lg">
                    <div className="flex items-start justify-between">
                      ...
                      <button className="text-red-500 hover:text-red-700" aria-label="Delete question">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                  </div>
